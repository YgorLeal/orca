...
Use literal repository-relative paths to ignored files or folders. Blank lines and whole-line `#` comments are allowed; globs and negation are not. The manifest is limited to 256 KiB and 1,000 entries. Tracked files already come from Git. Missing extras are reported and skipped; unsafe, nonignored, or conflicting selections need correction before preparation continues.
